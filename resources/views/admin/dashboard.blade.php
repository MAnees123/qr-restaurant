...
<div class="mb-8" x-data="smartSearch()">
    <div class="relative group">
        <div class="absolute inset-y-0 left-0 pl-6 flex items-center pointer-events-none text-slate-400 group-focus-within:text-emerald-500 transition-colors z-20">
        ...
        <input type="text" 
               ...
               placeholder="Smart Search: Order ID, Table #, or Date..." 
               class="w-full bg-white border-2 border-slate-100 rounded-[2rem] py-6 pl-16 pr-8 text-lg font-bold text-slate-800 placeholder-slate-300 focus:outline-none focus:border-emerald-500 focus:ring-0 transition shadow-sm hover:shadow-md z-10 relative">
        ...
        <div x-show="open && results.length > 0" 
            ...
            <div class="p-6 bg-slate-50 border-b flex justify-between items-center">
                <span class="text-xs font-black text-slate-400 uppercase tracking-widest" x-text="results.length + ' Matches Found'"></span>
                <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest bg-slate-200 px-2 py-1 rounded">Arrows to Navigate • Enter to View</span>
            </div>
                   ...
                   :class="selectedIndex === index ? 'bg-emerald-50 border-emerald-100' : 'hover:bg-slate-50'">
                    ...
                    <div class="flex-1">
                        <div class="flex justify-between items-start">
                            ...
                            <span class="text-xs font-black text-emerald-600" x-text="'Rs ' + parseFloat(order.total_amount).toLocaleString()"></span>
                        </div>
                        <div class="flex gap-3 items-center mt-1">
                            ...
                            <span class="text-[10px] font-black uppercase px-2 py-0.5 rounded bg-slate-100" 
                                  :class="{
                                    ...
                                    'text-emerald-500 bg-emerald-50': order.status === 'ready' || order.status === 'served'
        ...
        <div x-show="open && query.length > 2 && results.length === 0 && !loading" 
            ...
            <div class="bg-slate-100 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
                <svg class="w-8 h-8 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.172 9.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
            </div>
            <h3 class="font-black text-slate-800">No Orders Found</h3>
            <p class="text-sm text-slate-400 font-bold mt-1">Try searching with a different Order ID, Table #, or Date</p>
        </div>

        <button type="button" @click="search()" class="absolute right-4 top-4 bottom-4 bg-slate-900 text-white px-8 rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-emerald-600 transition shadow-lg z-20">
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    <a href="{{ route('admin.revenue') }}" class="bg-white rounded-[2rem] shadow-sm border p-8 transition duration-300 transform hover:-translate-y-1 hover:shadow-lg border-l-8 border-emerald-500 block">
        ...
        <div class="mt-4 flex gap-2">
            <span class="text-[10px] font-black bg-emerald-100 text-emerald-700 px-2 py-1 rounded">CASH: Rs {{ number_format($stats['cash_payments']) }}</span>
            <span class="text-[10px] font-black bg-blue-100 text-blue-700 px-2 py-1 rounded">ONLINE: Rs {{ number_format($stats['online_payments']) }}</span>
        </div>
    </a>
    
    <a href="{{ route('admin.orders.index') }}" class="bg-white rounded-[2rem] shadow-sm border p-8 transition duration-300 transform hover:-translate-y-1 hover:shadow-lg border-l-8 border-amber-500 block">
    ...
    <a href="{{ route('admin.orders.index') }}" class="bg-white rounded-[2rem] shadow-sm border p-8 transition duration-300 transform hover:-translate-y-1 hover:shadow-lg block">
    ...
    <a href="{{ route('admin.tables.index') }}" class="bg-white rounded-[2rem] shadow-sm border p-8 transition duration-300 transform hover:-translate-y-1 hover:shadow-lg block">
        ...
        <p class="text-[10px] font-bold text-blue-600 mt-2">{{ $stats['pending_reservations'] }} pending reservations</p>
    </a>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
    ...
    <div class="bg-white rounded-[2rem] shadow-sm border p-8">
        ...
        <div class="space-y-4">
            <a href="{{ route('admin.menu.download') }}" target="_blank" class="flex items-center gap-4 p-4 bg-emerald-50 rounded-2xl border border-emerald-100 hover:bg-emerald-100 transition group">
            ...
            <a href="{{ route('admin.discounts.create') }}" class="flex items-center gap-4 p-4 bg-amber-50 rounded-2xl border border-amber-100 hover:bg-amber-100 transition">
            ...
            <a href="{{ route('admin.menu-items.create') }}" class="flex items-center gap-4 p-4 bg-blue-50 rounded-2xl border border-blue-100 hover:bg-blue-100 transition">
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-[10px] text-slate-400 font-black uppercase">{{ $res->reservation_time }} • {{ $res->guests }} Guests</p>
                    </div>
                </div>
                @endforeach
            </div>
...
<div class="mb-8" x-data="waiterCalls()" x-init="initPolling()">
    <template x-if="calls.length > 0">
        <div class="bg-amber-50 border-2 border-amber-200 rounded-[2rem] p-6 shadow-lg animate-pulse hover:animate-none transition-all">
...
<div class="bg-white rounded-[2rem] shadow-sm border overflow-hidden" x-data="liveOrders()" x-init="initPolling()">
    <div class="p-8 border-b flex justify-between items-center bg-slate-50/50">
        ...
        <div class="flex items-center gap-3">
            <span class="flex items-center gap-2 px-4 py-2 bg-emerald-100 text-emerald-700 rounded-xl text-xs font-black uppercase tracking-widest">
    ...
    <div class="p-8 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 bg-white">
        <template x-for="order in orders" :key="order.id">
            <div class="bg-slate-50 rounded-[2rem] border border-slate-100 p-6 hover:shadow-xl hover:shadow-slate-200/50 transition duration-300 group relative overflow-hidden">
                ...
                <div class="flex items-start gap-4 mb-6">
                    <div class="w-14 h-14 rounded-2xl bg-white border border-slate-100 flex items-center justify-center font-black text-slate-800 shadow-sm group-hover:bg-emerald-600 group-hover:text-white transition duration-300">
                    ...
                    <div>
                        ...
                        <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest" x-text="formatTime(order.created_at)"></p>
                    </div>
                </div>
                ...
                <div class="space-y-3 mb-6">
                    <div class="flex justify-between items-center text-sm font-bold">
                        ...
                        <span class="text-slate-800" x-text="'Rs ' + parseFloat(order.total_amount).toLocaleString()"></span>
                    </div>
                    <div class="flex justify-between items-center text-sm font-bold">
                        <span class="text-slate-400">Payment</span>
                        <span class="px-3 py-1 text-[10px] font-black uppercase rounded-lg tracking-widest shadow-sm border"
                              :class="{
                                'bg-emerald-100 text-emerald-700 border-emerald-200': order.payment_status === 'paid',
                                'bg-red-50 text-red-600 border-red-100': order.payment_status === 'pending'
                              }" x-text="order.payment_status"></span>
                    </div>
                </div>

                <a :href="'/admin/orders/' + order.id" class="w-full flex items-center justify-center gap-2 py-3 bg-white border border-slate-200 rounded-xl font-black text-xs uppercase tracking-widest text-slate-600 hover:bg-slate-900 hover:text-white hover:border-slate-900 transition duration-300">
                    View Details
    ...
    function smartSearch() {
        return {
            ...
            search() {
                ...
                this.loading = true;
                this.open = true;
                axios.get('{{ route("admin.orders.live-search") }}', { params: { q: this.query } })
                    .then(response => {
